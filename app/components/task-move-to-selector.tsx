...
import { LuInbox, LuList } from "react-icons/lu";
...
export function TaskMoveToSelector({
  ...
  }, [currentListId, lists, trimmedQuery]);
      ...
      <div className="max-h-[240px] overflow-y-auto py-1">
        {filteredLists.length === 0 ? (
          <p className="px-4 py-3 text-sm text-zinc-400 dark:text-zinc-500">
            {trimmedQuery ? "No matching lists" : "No other lists"}
          </p>
        ) : (
          filteredLists.map((list) => {
            ...
            return (
              <button
                ...
                className="flex h-[38px] w-full items-center gap-2.5 px-4 text-left text-sm text-zinc-800 transition-colors hover:bg-zinc-50 dark:text-zinc-100 dark:hover:bg-zinc-800/70"
              >
